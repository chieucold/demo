...
class BNDDateTimePicker extends FormBuilderField<DateTime> {
  ...
  BNDDateTimePicker(
      ...
      : super(
          ...
          builder: (FormFieldState<DateTime?> field) {
            final _BNDDateTimePickerState state = field as _BNDDateTimePickerState;

            late InputDecoration decoration =
                state.decoration().copyWith(border: OutlineInputBorder(borderRadius: BorderRadius.circular(70)));

            if (backgroundColor != null) decoration = decoration.copyWith(filled: true, fillColor: backgroundColor);
            if (contentPadding != null) decoration = decoration.copyWith(contentPadding: contentPadding);
            if (leftIcon != null) decoration = decoration.copyWith(prefixIcon: leftIcon);
            if (rightIcon != null)
  ...
  @override
  _BNDDateTimePickerState createState() => _BNDDateTimePickerState();
}

class _BNDDateTimePickerState extends FormBuilderFieldState<BNDDateTimePicker, DateTime> {
  late TextEditingController _textFieldController;
  late bool _isShowAfterTime;
  late DateFormat _dateFormat;

  @override
  void initState() {
    super.initState();
    _isShowAfterTime = widget.isShowAfterTime;
    _textFieldController = widget.controller ?? TextEditingController();
    _dateFormat = widget.format ?? _getDefaultDateTimeFormat();
    final DateTime? initVal = initialValue;
  ...
  Future<DateTime?> onShowPicker(BuildContext context, DateTime? currentValue) async {
    ...
    switch (widget.inputType) {
      ...
      case InputType.time:
        ...
        break;
      case InputType.both:
        final DateTime? date = await _showDatePicker(context, currentValue);
        if (date != null) {
          final TimeOfDay? time = await _showTimePicker(context, currentValue);
    ...
    DateTime? finalValue = newValue ?? currentValue;
    if (finalValue != null) {
      if (widget.inputType == InputType.date) finalValue = DateTime(finalValue.year, finalValue.month, finalValue.day);
    }
  ...
  Future<DateTime?> _showDatePicker(BuildContext context, DateTime? currentValue) {
    final DateTime _minDate =
        _isShowAfterTime != null && !_isShowAfterTime ? DateTime.now() : widget.firstDate ?? DateTime(1900);
    if (Platform.isIOS)
  ...
  LocaleType getLocaleType() {
    return Utils.enumFromString<LocaleType>(LocaleType.values, AppLocalizations.of(context)!.locale.languageCode);
  }
