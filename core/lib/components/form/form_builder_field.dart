...
import 'package:core/components/form/form_builder.dart';
...
class FormBuilderFieldState<F extends FormBuilderField<T?>, T> extends FormFieldState<T?> {
  ...
  bool get isDate => typeOf<T>() == typeOf<DateTime>();
  ...
  T? get initialValue {
    if (widget.initialValue != null)
      ...
      if (isDate)
        return DateTime.tryParse((_formBuilderState?.initialValue ?? const <String, dynamic>{})[widget.name].toString())
            as T?;
      else
        return (_formBuilderState?.initialValue ?? const <String, dynamic>{})[widget.name] as T?;
    }
  }
    ...
    if (_formBuilderState != null) {
      if (enabled || !_formBuilderState!.widget.skipDisabled) {
        _formBuilderState!.setInternalFieldValue(
          widget.name,
          null != widget.valueTransformer ? widget.valueTransformer!(value) : (isDate ? value.toString() : value),
        );
      } else {
