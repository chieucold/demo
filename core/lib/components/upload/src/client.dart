...
class TusClient {
  ...
  Future<bool> create() async {
    ...
    final createHeaders = Map<String, String>.from(headers ?? <String, String>{})
      ..addAll({
    ...
    if (!(response.statusCode >= 200 && response.statusCode < 300) && response.statusCode != 404) {
      throw ProtocolException("unexpected status code (${response.statusCode}) while creating upload");
    }
  ...
  Future<void> upload({
    ...
    while (!_pauseUpload && (_offset ?? 0) < totalBytes) {
      final uploadHeaders = Map<String, String>.from(headers ?? <String, String>{})
        ..addAll({
          "Tus-Resumable": tusVersion,
      ...
      final http.Response response = await _chunkPatchFuture;
      _chunkPatchFuture = null;

      // check if correctly uploaded
      if (!(response.statusCode >= 200 && response.statusCode < 300)) {
        throw ProtocolException("unexpected status code (${response.statusCode}) while uploading chunk");
      }
  ...
  String generateMetadata() {
    final meta = Map<String, String>.from(metadata ?? <String, String>{});
  ...
  Future<int> _getOffset() async {
    ...
    final offsetHeaders = Map<String, String>.from(headers ?? <String, String>{})
      ..addAll({
        "Tus-Resumable": tusVersion,
      });
    ...
    if (!(response.statusCode >= 200 && response.statusCode < 300)) {
      throw ProtocolException("unexpected status code (${response.statusCode}) while resuming upload");
    }
