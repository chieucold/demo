...
final double _kMaxSwipeDistance = 0.6; // As portion of screen.
...
mixin CupertinoRouteTransitionMixin<T> on PageRoute<T> {
  ...
  static bool _isPopGestureEnabled<T>(PageRoute<T> route) {
    ...
    if (!route.willDisposeAnimationController) return false;
...
class _CupertinoBackGestureDetectorState<T> extends State<_CupertinoBackGestureDetector<T>> {
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    if (_willPopCallback != null && details.globalPosition.dx >= context.size!.width) {
      _handleDragCancel();
    }
  ...
  void _handleDragEnd(DragEndDetails details) {
    ...
    _backGestureController?.dragEnd(
        _convertToLogical(details.velocity.pixelsPerSecond.dx / context.size!.width * _kMaxSwipeDistance),
        _willPopCallback);
  ...
  void _handleDragCancel() {
    ...
    _backGestureController?.dragEnd(0.0, _willPopCallback);
    _backGestureController = null;
  }
...
class _CupertinoBackGestureController<T> {
  ...
  void dragEnd(double velocity, Future<RoutePopDisposition> Function()? onPop) async {
    ...
    if (animateForward) {
      print('AnimateForward Not Ended');
      ...
      print(' AnimateForward Ended Swipe');

      // This route is destined to pop at this point. Reuse navigator's pop.
      if (onPop != null) {
        final RoutePopDisposition data = await onPop();
        if(data.index == RoutePopDisposition.doNotPop.index){
          print('Pop Has Dialog');
          final int droppedPageForwardAnimationTime = min(
            lerpDouble(_kMaxDroppedSwipePageForwardAnimationTime, 0, controller.value)!.floor(),
            _kMaxPageBackAnimationTime,
          );
